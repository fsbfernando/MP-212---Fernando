                    ...
                    if isfinite(Z) && Z>0
                        ...
                        if ~isempty(poseMsg)
                            ...
                            try
                                ...
                                % ================================
                                % [NOVO — APENAS PRINTS] RPY em câmera e base (graus)
                                % ================================
                                % --- Frame da CÂMERA ---
                                x_c = eixo_longo_cam(:);  x_c = x_c / max(norm(x_c),1e-12);
                                z_c = eixo_normal_cam(:); z_c = z_c / max(norm(z_c),1e-12);
                                y_c = cross(z_c, x_c);   y_c = y_c / max(norm(y_c),1e-12);
                                z_c = cross(x_c, y_c);   z_c = z_c / max(norm(z_c),1e-12);
                                R_cam_bag = [x_c y_c z_c];
                                % Euler ZYX → [yaw pitch roll], depois reordenamos para [roll pitch yaw]
                                pitch_c = -asin(max(-1,min(1, R_cam_bag(3,1))));
                                yaw_c   = atan2(R_cam_bag(2,1), R_cam_bag(1,1));
                                roll_c  = atan2(R_cam_bag(3,2), R_cam_bag(3,3));
                                rpy_cam_deg = rad2deg([roll_c, pitch_c, yaw_c]);
                                fprintf('[RPY_cam_deg]  roll=%.2f  pitch=%.2f  yaw=%.2f\n', rpy_cam_deg(1), rpy_cam_deg(2), rpy_cam_deg(3));

                                % --- Frame BASE do robô ---
                                x_b = eixo_longo_base(:);  x_b = x_b / max(norm(x_b),1e-12);
                                z_b = eixo_normal_base(:); z_b = z_b / max(norm(z_b),1e-12);
                                y_b = cross(z_b, x_b);     y_b = y_b / max(norm(y_b),1e-12);
                                z_b = cross(x_b, y_b);     z_b = z_b / max(norm(z_b),1e-12);
                                R_base_bag = [x_b y_b z_b];
                                pitch_b = -asin(max(-1,min(1, R_base_bag(3,1))));
                                yaw_b   = atan2(R_base_bag(2,1), R_base_bag(1,1));
                                roll_b  = atan2(R_base_bag(3,2), R_base_bag(3,3));
                                rpy_base_deg = rad2deg([roll_b, pitch_b, yaw_b]);
                                fprintf('[RPY_base_deg] roll=%.2f  pitch=%.2f  yaw=%.2f\n', rpy_base_deg(1), rpy_base_deg(2), rpy_base_deg(3));
                                fprintf('---\n');
